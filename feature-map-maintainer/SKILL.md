...
description: Maintain `.ai/features-index.yaml` and `.ai/features-details/` directly from staged git changes. Use when checking whether the current commit makes the map stale and updating only the affected entries.
disable-model-invocation: true
---

# Feature Map Maintainer

Use this skill to keep `.ai/features-index.yaml` and `.ai/features-details/` current from staged git changes. Treat the map as the working source of truth for feature-oriented code navigation.

Read the current map and the staged git diff, then decide whether the map needs to change. Update only the affected entries unless the staged changes clearly introduce a genuinely new feature area, in which case add a new entry.
...
1. Read `<target-repo>/.ai/features-index.yaml`. If it does not exist, stop and explain that the map must exist before maintenance can run.
...
4. Match staged changes against the existing map entries in the index first.
5. For affected entries, read their `<target-repo>/.ai/features-details/<name>.yaml` file. Update `description` in the index entry and `core_files` / `supporting_files` in the detail file only when the staged diff shows the map has become stale.
...
9. If the staged changes clearly introduce a genuinely new feature area that is not represented in the map, append a new entry to `features-index.yaml` and create a new `features-details/<name>.yaml` with the same `core_files` / `supporting_files` shape.
